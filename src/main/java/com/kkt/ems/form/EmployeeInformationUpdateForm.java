...
public class EmployeeInformationUpdateForm {
	...
	/** 社員名(漢字) */
	@Pattern(regexp = "([ぁ-んァ-ヶー一-龠]+　[ぁ-んァ-ヶー一-龠]+)", message = "社員名(漢字) : 姓名の間に全角スペースを入れ、全角カナひら漢字で入力してください")
	private String shainNameKj;

	/** 社員名(カナ) */
	@Pattern(regexp = "([ア-ン゛゜ァ-ォャ-ョー「」、]+　[ア-ン゛゜ァ-ォャ-ョー「」、]+)", message = "社員名(カナ) : 姓名の間に全角スペースを入れ、全角カナで入力してください")
	private String shainNameKn;

	/** 部署名 */
	@Pattern(regexp = "^(?!.*選択してください).+$", message = "所属部署 : 選択してください")
